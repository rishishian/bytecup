...
if __name__ == '__main__':
    zip_file_name_list = ['bytecup.corpus.train.%d.zip' % i for i in range(9)]
    zip_file_name_list.append('bytecup.corpus.validation_set.zip')

    for zip_file_name in zip_file_name_list:
        print(zip_file_name)

        print('zip file name: %s' % zip_file_name)
        zip_file_path = os.path.join(opt.root_path, zip_file_name)
        ...
        print('finish preprocessing %s' % zip_file_name)
    print('total num for training:%d'%total_num)
